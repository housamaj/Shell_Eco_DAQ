...
unsigned long MPH; // MPH using wheel diameter
...
void setup()  // Start of setup:
{
  ...
  attachInterrupt(digitalPinToInterrupt(2), Pulse_Event, RISING);  // Enable interruption pin 7 when going from LOW to HIGH.
...
void loop()  // Start of loop:
{
  ...
  float diameter = 19; // Diameter in inches
  float radius = (diameter/2)/12; // Radius in feet
  float MPH = RPM * ((60*2*PI*radius)/5280); // Convert from RPM to MPH
  ...
  // Set the cursor to column 0, line 0 (top row) for MPH
  lcd.setCursor(0, 0);
  lcd.print("MPH: ");
  lcd.print(MPH);
  lcd.print("        ");  // Add spaces to clear out any previous characters if the MPH value decreases in length.

  // Set the cursor to column 0, line 1 (bottom row) for RPM
  lcd.setCursor(0, 1);
  lcd.print("RPM: ");
  lcd.print(RPM);
  lcd.print("   ");  // Add spaces to clear out any previous characters if the RPM value decreases in length.
